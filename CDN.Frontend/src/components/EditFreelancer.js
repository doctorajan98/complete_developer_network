...
const EditFreelancer = () => {
  ...
  const fetchFreelancer = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await freelancerAPI.getFreelancerById(id);
      setFormData({
        userId: data.userId,
      ...
      setError('Failed to fetch freelancer details. Please try again.');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await freelancerAPI.updateFreelancer(formData);
      setSuccess(true);
      setTimeout(() => {
        navigate('/');
      }, 2000);
    } catch (err) {
      setError(err.response?.data || 'Failed to update freelancer. Please try again.');
    ...
    <Box>
      ...
      {success && (
        <Alert severity="success" sx={{ mb: 2 }}>
          Freelancer updated successfully! Redirecting to the list...
        </Alert>
      )}
            ...
            <Grid container spacing={3}>
              ...
              <Grid size={12}>
                <Box display="flex" gap={2} justifyContent="flex-end" sx={{ mt: 3 }}>
                  ...
                  >
                    {saving ? 'Updating...' : 'Update Freelancer'}
                  </Button>
                </Box>
              </Grid>
            </Grid>
